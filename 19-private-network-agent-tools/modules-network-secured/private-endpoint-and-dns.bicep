...
@description('Azure region for private endpoints. Must match the virtual network region.')
...
@description('Optional full ARM ID of the private endpoint subnet. When set, private endpoints are placed in this subnet instead of resolving `peSubnetName` under `vnetName`.')
...
var peSubnetResourceIdPassedIn = peSubnetResourceId != ''
resource peSubnet 'Microsoft.Network/virtualNetworks/subnets@2024-05-01' existing = if (!peSubnetResourceIdPassedIn) {
  parent: vnet
  name: peSubnetName
}
var effectivePeSubnetResourceId = peSubnetResourceIdPassedIn ? peSubnetResourceId : peSubnet!.id
...
resource fabricPrivateEndpoint 'Microsoft.Network/privateEndpoints@2024-05-01' = if (fabricPassedIn) {
  ...
  tags: resourceTags
  properties: {
    subnet: { id: effectivePeSubnetResourceId } // Deploy in customer private endpoint subnet
    privateLinkServiceConnections: [
      {
