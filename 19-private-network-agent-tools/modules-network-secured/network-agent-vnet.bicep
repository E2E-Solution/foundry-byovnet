...
@description('Optional full ARM Resource ID of an existing private endpoint subnet in a different VNet. When set, private endpoints use this subnet and the main VNet PE subnet is not managed.')
param externalPeSubnetResourceId string = ''

@description('Tags applied to taggable network resources created by this module.')
...
output peSubnetName string = externalPeSubnetResourceId != '' ? last(split(externalPeSubnetResourceId, '/')) : peSubnetName
...
output peSubnetId string = useExistingVnet ? existingVNet!.outputs.peSubnetId : (externalPeSubnetResourceId != '' ? externalPeSubnetResourceId : newVNet!.outputs.peSubnetId)
