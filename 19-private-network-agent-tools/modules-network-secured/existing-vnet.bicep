...
@description('Optional full ARM Resource ID of an existing private endpoint subnet in a different VNet. When set, this module does not create or modify a PE subnet in the main VNet.')
param externalPeSubnetResourceId string = ''

// Get the address space (array of CIDR strings)
var vnetAddressSpace = existingVNet.properties.addressSpace.addressPrefixes[0]
var peSubnetIsExternal = externalPeSubnetResourceId != ''
...
module peSubnet 'subnet.bicep' = if (!peSubnetIsExternal) {
...
module mcpSubnetAfterPeSubnet 'subnet.bicep' = if (!peSubnetIsExternal) {
...
// When the PE subnet is external, continue serializing writes to this VNet by
// creating/referencing MCP only after the agent subnet operation completes.
module mcpSubnetAfterAgentSubnet 'subnet.bicep' = if (peSubnetIsExternal) {
  name: 'mcp-subnet-external-pe-${uniqueString(deployment().name, mcpSubnetName)}'
  scope: resourceGroup(vnetResourceGroupName)
  params: {
    vnetName: vnetName
    subnetName: mcpSubnetName
...
// Output variables
output peSubnetName string = peSubnetIsExternal ? last(split(externalPeSubnetResourceId, '/')) : peSubnetName
...
output peSubnetId string = peSubnetIsExternal ? externalPeSubnetResourceId : peSubnet!.outputs.subnetId
output mcpSubnetId string = peSubnetIsExternal ? mcpSubnetAfterAgentSubnet!.outputs.subnetId : mcpSubnetAfterPeSubnet!.outputs.subnetId
