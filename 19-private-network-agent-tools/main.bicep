...
@description('Tags applied to taggable resources created by this template. The default `SecurityControl=Ignore` tag can be used as an approved governance exclusion for MCAPS diagnostic settings policy.')
param resourceTags object = {
  SecurityControl: 'Ignore'
}

@description('Optional Microsoft Entra object ID to receive the Foundry Project Manager role at both the Foundry account and project scopes. Leave empty to skip this assignment.')
param foundryProjectManagerPrincipalId string = ''

@description('Principal type for `foundryProjectManagerPrincipalId`. Use `User` for a user object ID, `Group` for a group object ID, and `ServicePrincipal` for an app or managed identity object ID.')
@allowed([
  'User'
  'Group'
  'ServicePrincipal'
...
param foundryProjectManagerPrincipalType string = 'User'

@description('Optional role assignment resource name for the account-scope Foundry Project Manager assignment. Leave empty to use a deterministic GUID.')
param foundryProjectManagerAccountRoleAssignmentName string = ''

@description('Optional role assignment resource name for the project-scope Foundry Project Manager assignment. Leave empty to use a deterministic GUID. Set this when adopting a pre-existing manual assignment.')
param foundryProjectManagerProjectRoleAssignmentName string = ''
...
@description('Optional ARM Resource ID of an existing private endpoint subnet. If provided, the subnet will be referenced as-is and will not be created/modified. It may be in a different VNet from `existingVnetResourceId`, but it must be in the same Azure region as `location`.')
...
var peSubnetParts = split(existingPeSubnetResourceId, '/')
var effectiveAgentSubnetName = agentSubnetExists ? last(split(existingAgentSubnetResourceId, '/')) : agentSubnetName
var effectivePeSubnetName    = peSubnetExists    ? peSubnetParts[10]    : peSubnetName
...
var peSubnetVnetSubscriptionId = peSubnetExists ? peSubnetParts[2] : vnetSubscriptionId
var peSubnetVnetResourceGroupName = peSubnetExists ? peSubnetParts[4] : vnetResourceGroupName
var peSubnetVnetName = peSubnetExists ? peSubnetParts[8] : trimVnetName
var peSubnetIsExternal = peSubnetExists && (toLower(peSubnetVnetSubscriptionId) != toLower(vnetSubscriptionId) || toLower(peSubnetVnetResourceGroupName) != toLower(vnetResourceGroupName) || toLower(peSubnetVnetName) != toLower(trimVnetName))
var externalPeSubnetResourceId = peSubnetIsExternal ? existingPeSubnetResourceId : ''
...
module vnet 'modules-network-secured/network-agent-vnet.bicep' = {
  ...
  params: {
    ...
    peSubnetExists: peSubnetExists && !peSubnetIsExternal
    mcpSubnetExists: mcpSubnetExists
    externalPeSubnetResourceId: externalPeSubnetResourceId
...
module privateEndpointAndDNS 'modules-network-secured/private-endpoint-and-dns.bicep' = {
  ...
  params: {
    ...
    peSubnetResourceId: vnet.outputs.peSubnetId // Supports PE subnet in a separate existing VNet
...
module foundryProjectManagerRoleAssignments 'modules-network-secured/foundry-project-manager-role-assignments.bicep' = if (foundryProjectManagerPrincipalId != '') {
  name: 'foundry-project-manager-ra-${uniqueSuffix}-deployment'
  params: {
    accountName: aiAccount.outputs.accountName
    projectName: aiProject.outputs.projectName
    principalId: foundryProjectManagerPrincipalId
    principalType: foundryProjectManagerPrincipalType
    accountRoleAssignmentName: foundryProjectManagerAccountRoleAssignmentName
    projectRoleAssignmentName: foundryProjectManagerProjectRoleAssignmentName
  }
}
