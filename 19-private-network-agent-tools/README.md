...
#### Southeast Asia existing network example

Use [`main.charlie-rg-southeastasia-existing-network.bicepparam`](./main.charlie-rg-southeastasia-existing-network.bicepparam) when you want to deploy a new Foundry account in `southeastasia` while reusing existing Charlie network assets and private DNS zones.

Before deployment, review and amend these values:

- Keep `location = 'southeastasia'` and use Class B or Class C CIDR ranges for any new agent or MCP subnet. For Southeast Asia, avoid creating agent-service subnets in `10.0.0.0/8` unless the service team confirms support for that address class in your tenant and region.
- Set `existingVnetResourceId` to the VNet that hosts the Foundry agent subnet and, when needed, the MCP subnet.
- Set `existingAgentSubnetResourceId` when the agent subnet already exists. The subnet must be dedicated to one Foundry account and delegated to `Microsoft.App/environments`.
- Set `existingPeSubnetResourceId` to the full ARM ID of the subnet where private endpoint NICs should be created. This subnet can be in a separate VNet, such as a hub or VM access VNet, as long as it is in the same region as the Foundry deployment.
- Set `existingMcpSubnetResourceId` only when that subnet already exists in the same VNet as `existingVnetResourceId`. Leave it empty and provide `mcpSubnetPrefix` when you want the template to create the missing MCP subnet.
- Set each `existingDnsZones` entry to the resource group that owns the zone. When a zone is referenced from another resource group, this template does not create the VNet link.
- Pre-link each referenced private DNS zone to the VNet that hosts the private endpoint subnet. Also link the zone to any peered VNet whose clients must resolve private endpoint names.
- Decide whether to reuse `existingAiSearchResourceId`, `existingAzureStorageAccountResourceId`, and `existingAzureCosmosDBAccountResourceId`. If you reuse existing data resources, deploy into a separate resource group to avoid private endpoint name collisions with existing deployments.

For the current Alpha and Charlie Southeast Asia environment, `main.charlie-rg-southeastasia-existing-network.bicepparam` uses `charlie-foundry-vnet/snet-agent-services-foundry2` for Foundry agent network injection and `alpha-azvm-vnet/snet-private-endpoints` for private endpoint NIC creation. The existing private DNS zones in `alpha-foundry-rg` are already linked to `alpha-azvm-vnet`. If clients in `charlie-foundry-vnet` also need DNS resolution, create additional VNet links from those zones to `charlie-foundry-vnet` before deployment.

Example VNet link command:

```bash
az network private-dns link vnet create \
  --resource-group charlie-foundry-rg \
  --zone-name privatelink.openai.azure.com \
  --name privatelink-openai-charlie-foundry-vnet-link \
  --virtual-network /subscriptions/27b58ab1-f52c-498e-a03d-255b76c80a00/resourceGroups/charlie-foundry-rg/providers/Microsoft.Network/virtualNetworks/charlie-foundry-vnet \
  --registration-enabled false
```

Repeat the link command for these zones when they are referenced through `existingDnsZones`:
...
After amending the parameter file, validate and preview the deployment:

```bash
az deployment group validate \
  --resource-group <target-resource-group> \
  --template-file main.bicep \
  --parameters main.charlie-rg-southeastasia-existing-network.bicepparam
